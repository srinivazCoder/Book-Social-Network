...
import com.srinivas.Book.book.BookRepository;
import com.srinivas.Book.book.PageResponse;
...
public class FeedbackService {
    ...
    public Integer save(FeedbackRequest request, Authentication connectedUser) {
        ...
        if(book.isArchived() || !book.isShareable()){
            throw new OperationNotPermittedException("You can not give a feedback for archived or not shareable book");

        }
        User user = ((User) connectedUser.getPrincipal());

        if(Objects.equals(book.getOwner().getId(), user.getId())){
            throw new OperationNotPermittedException("You cannot give a feedback to you own book");
